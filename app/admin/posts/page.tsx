    ...
    <div className={styles.shell}>
      ...
      <main className={styles.main} style={{ maxWidth: 680 }}>
        ...
        <div className={styles.toolbar}>
          <form className={styles.search} action="/admin/posts">
            ...
            {query && <Link className={styles.clearSearch} href="/admin/posts" aria-label="検索を解除">×</Link>}
          </form>
        ...
        {totalPages > 1 && (
          <nav className={styles.pagination} aria-label="ページ送り">
            {page > 1 && <Link href={`/admin/posts?page=${page - 1}${queryPart}`} className={styles.pageButton}>← 前へ</Link>}
            <span className={styles.pageCount}>{page} / {totalPages}</span>
            {page < totalPages && <Link href={`/admin/posts?page=${page + 1}${queryPart}`} className={styles.pageButton}>次へ →</Link>}
          </nav>
        )}
      </main>
    </div>
