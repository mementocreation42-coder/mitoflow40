...
import { getSymptomBySlug, symptoms } from '@/lib/symptoms';
...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const name = s?.name ?? 'SYMPTOMS';
    const en = s?.en ?? '';
    const tagline = s?.tagline ?? '';
    const bg = s?.color ?? '#F0E7E0';
        ...
        (
            <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', background: bg, padding: '70px 80px', fontFamily: 'sans-serif' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 30, fontWeight: 700, color: '#1A1A1A' }}>
                    <div>Mitoflow40</div>
                    <div style={{ fontSize: 22, color: '#FF9855', letterSpacing: 4 }}>FROM SYMPTOMS ／ 症状</div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <div style={{ fontSize: 76, fontWeight: 800, color: '#1A1A1A', lineHeight: 1.1 }}>{name}</div>
                    <div style={{ fontSize: 26, color: '#1A1A1A', opacity: 0.55, marginTop: 6 }}>{en}</div>
                    <div style={{ fontSize: 28, color: '#1A1A1A', opacity: 0.8, marginTop: 18 }}>{tagline}</div>
                </div>
            </div>
        ),
        { ...size }
